    ...
    public class JsonMapper
    {
        ...
        private static object ReadValue (Type inst_type, JsonReader reader)
        {
            ...
            } else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                else if (typeof(ScriptableObject).IsAssignableFrom(value_type))
                    instance = ScriptableObject.CreateInstance(value_type);
                else 
            ...
            foreach(var i in typeof(JsonMapper).GetMethods())
            {
                if(i.Name == "ToObject")
                {
                    if (i.IsGenericMethodDefinition) {
                        ...
                        var param = i.GetParameters();
                        if (param.Length == 2) {
                            if (param[0].ParameterType == typeof(Type) && param[1].ParameterType == typeof(string)) {
                                appdomain.RegisterCLRMethodRedirection(i, JsonToObject4);
                            }
                        }
                    }
                }
        ...
        public unsafe static StackObject* JsonToObject4(ILIntepreter intp, StackObject* esp, IList<object> mStack, CLRMethod method, bool isNewObj)
        {
            ILRuntime.Runtime.Enviorment.AppDomain __domain = intp.AppDomain;
            StackObject* ptr_of_this_method;
            ...
            System.Type t = (System.Type)typeof(System.Type).CheckCLRTypes(StackObject.ToObject(ptr_of_this_method, __domain, mStack));
            intp.Free(ptr_of_this_method);

            var type = __domain.LoadedTypes[t.FullName].ReflectionType;
            var result_of_this_method = ReadValue(type, new JsonReader(json));
